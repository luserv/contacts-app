...
export default function DocumentsCard({ contactId, initialCards }: Props) {
  const { t } = useI18n();
  const { getIdentityCards, addIdentityCard, deleteIdentityCard } = useContacts();
  ...
  const handleSave = async () => {
    ...
    const ok = await addIdentityCard(
      contactId, docType, cardNumber.trim(),
      issueDate.trim() || undefined, expiryDate.trim() || undefined,
    );
    if (ok) {
      ...
      <Modal visible={modalVisible} animationType="slide" transparent onRequestClose={() => setModalVisible(false)}>
        <View style={s.modalOverlay}>
          <View style={s.modalContainer}>
            <Text style={s.modalTitle}>{t.documents.addTitle}</Text>
